					...
					<div class="comment-metadata">
						<a href="<?php echo esc_url( get_comment_link( $comment, $args ) ); ?>">
							<?php
								...
								$comment_timestamp = sprintf( __( '%1$s at %2$s', 'mythos' ), get_comment_date( '', $comment ), get_comment_time() );
							?>
							<time datetime="<?php comment_time( 'c' ); ?>" title="<?php echo esc_attr( $comment_timestamp ); ?>">
								<?php echo esc_attr( $comment_timestamp ); ?>
							</time>
						</a>
						<?php edit_comment_link( __( 'Edit', 'mythos' ), '<span class="edit-link-sep">&mdash;</span> <span class="edit-link"></span>' ); ?>
						<?php
							comment_reply_link(
								array_merge(
									$args,
