...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="featured-image" style="background-image: url('<?php echo esc_url( $img_url ); ?>');">
	    ...
	    <div class="entry-meta">
	        <?php
			$enable_archive_author = get_theme_mod( 'magic_blog_enable_archive_author', true );
	        if ( $enable_archive_author ) {
				magic_blog_post_author(); 
			}

			$archive_date_enable = get_theme_mod( 'magic_blog_enable_archive_date', true );
			if ( $archive_date_enable ) {
				magic_blog_posted_on(); 
			}
			?>
            ...
            <?php
				...
				if ( 'excerpt' === $archive_content_type ) {
			        ...
			        <div class="read-more-link">
					    <a href="<?php the_permalink(); ?>"><?php echo esc_html( get_theme_mod( 'magic_blog_archive_excerpt', esc_html__( 'View the post', 'magic-blog' ) ) ); ?></a>
					</div><!-- .read-more -->
				<?php
				} else {
					the_content( sprintf(
